...
const MariReport = () => {
  ...
  const handleSelectTimePeriod = (value) => {
    console.log('value: ', value);
    setTimePeriod(value);
  };
  ...
  const onSearch = async () => {
    ...
    let _buckets = _get(
      response.data,
      'result_data.aggregations.2.buckets',
      []
    );
    console.log('===== Buckets ====');
  ...
  const handleBucketsV2 = (buckets = []) => {
    ...
    CHART_CHANNELS.forEach((obj) => {
      ...
      if (_bucket_obj) {
        let buckets_channel = _get(_bucket_obj, '3.buckets', []);
        ...
        buckets_channel.forEach((bc) => {
          let _value = _get(bc, '1.value');
          let _doc_count = _get(bc, 'doc_count');
          let _value_percent = Number((_value / _doc_count) * 100).toFixed(2);
          _data.push({
        ...
        let error = _get(_bucket_obj, '1.value');
                ...
                <div>
                  <Row gutter={16} key={index}>
                    ...
                    <Col span={6}>
                      <Card key={index}>
                        <Statistic
                          title="Time Out (Duration Time > 4000ms)"
            ...
            {dateStrFilter.length > 0 && (
              <>
                ...
                >
                  ...
                  <Tag color="#2db7f5">{dateStrFilter[0]}</Tag>-{' '}
